...
import static me.gravityio.goodlib.dev.AnsiCodes.*;
...
public class CommandProcessor {
    ...
    public static void register(Class<?> clazz) {
        ...
        if (server) {
            GoodLib.LOGGER.debug("Registering Server Sided Commands from Class '{}'", clazz.getSimpleName());
            var cmd = processServer(clazz, commands);
            serverCommands.add(cmd);
        } else {
            GoodLib.LOGGER.debug("Registering Client Sided Commands from Class '{}'", clazz.getSimpleName());
    ...
    public static LiteralArgumentBuilder<ServerCommandSource> processServer(Class<?> ass, Commands classAnnon) {
        GoodLib.LOGGER.debug("{}Processing Server Sided Commands from Class {}'{}'{}", GR, BG, ass.getSimpleName(), RST);
        ...
        for (Method method : commandAnnon) {
            ...
            GoodLib.LOGGER.debug("  {}Compiling method command {}'{}'{} from method '{}'{}", BG, GR, cmdName, BG, method.getName(), RST);
    ...
    public static LiteralArgumentBuilder<FabricClientCommandSource> processClient(Class<?> ass, Commands classAnnon) {
        GoodLib.LOGGER.debug("Processing Client Sided Command from Class '{}'", ass.getSimpleName());
        String namespace = ass.getSimpleName().toLowerCase();
        if (classAnnon != null)
            namespace = classAnnon.namespace();
        ...
        for (Method method : commandAnnon) {
            ...
            if (cmdName.equals(""))
                cmdName = method.getName();
            GoodLib.LOGGER.debug("Registering command ({}) from method '{}'", cmdName, method.getName());
    ...
    private static Object invokeCommandServer(Method method, Map<Class<?>, IArgumentHandler<ServerCommandSource>> handlers, CommandContext<ServerCommandSource> context) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (handler == null) {
                ...
                GoodLib.LOGGER.error(er.toString());
                throw er;
            }
